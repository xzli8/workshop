...
\datedsubsection{\textbf{BNTO推荐系统工程架构从0到1：契约 $\cdot$ 主链路 $\cdot$ 状态层 $\cdot$ 实验归因 $\cdot$ 数据闭环}}{技术负责人}
\begin{onehalfspacing}
...
\textbf{工作内容}：本人担任技术负责人(带2名同学，与算法团队2人协作)，负责业内方案调研、HLD/LLD撰写、主链路+状态层+实验框架开发、混排与问卷卡接入，按端到端骨架$\rightarrow$实验框架$\rightarrow$状态层$\rightarrow$接入算法排序四个步骤逐层落地；一位同学负责客户端行为埋点$\rightarrow$Kafka$\rightarrow$ClickHouse/Flink的离线与实时特征数据链路，一位同学负责问卷卡的定向、频控与提交闭环。\\

\begin{itemize}
    \item \textbf{契约：定义工程与算法的边界}。排序请求只传候选身份(type+id+召回源)，特征由算法按key自取，保证训练与服务一致；召回归工程(索引是基建、候选集是资格控制)，算法以离线产物进入召回索引。两侧独立迭代，互不影响。
    \item \textbf{主链路：编排固定、策略可插拔}。多路并行召回recall(每路独立超时与线程池隔离、单路失败跳过、按type+id合并去重并取召回源并集、下发/曝光历史过滤)$\rightarrow$算法模型排序rank(RPC调算法，超时或失败降级为召回原序)$\rightarrow$重排策略链rerank(品牌/品类滑动窗口打散、来源分段等)$\rightarrow$混排blend(坑位规划、配额、瀑布流高度账本，品牌/合集/问卷/堆叠四类卡片)$\rightarrow$按type批量填充fill(填不出即丢)。新增一路召回、一条策略、一类卡片均为一个类加一行注册，主链路零改动。
    \item \textbf{状态层：cursor+cache}。首次请求跑全链路生成一个batch整体缓存(分页前全量有序的type+id列表与实验分组快照)，batch内翻页根据cursor查cache切片下发，不重跑全链路；去重按记忆时长分三层(batch内合并、session内已下发、跨session已曝光)，在召回合并后一次过滤；每层状态都有降级路径(cursor失效视为miss重建、缓存不可用走全链路、去重不可用放宽)。效果：batch内翻页不重跑上游，任何状态层故障只损失去重与缓存，页面永远有降级兜底。
    \item \textbf{实验归因与数据闭环}。实验：扁平模型，实验间正交；分组与准入在入口一次判定并冻结，保证归因可信；策略挂固定分流点：代码只注册策略，实验配置决定各组走哪个策略，新实验不发版。归因：exp\_group随卡片下发、随埋点原样回流，按组比对效果。数据：客户端事件写一次Kafka三路消费——ClickHouse落明细供T+1离线特征与按组实验分析，Flink窗口聚合session内行为为实时特征供排序请求期读取，曝光事件回流做跨session去重。
    \item \textbf{效果}：首个A/B实验(算法排序 vs 召回原序)商品卡点击率+18\%、加购率+11\%；首页全链路TP99约200ms，翻页TP99<50ms，页面成功率99.99\%；上线后一个月新增3类卡片(brand/collection/survey)、3条重排策略(品牌/品类打散、来源分段、下钻路径剔除)，平均2天上线。
\end{itemize}
\end{onehalfspacing}
