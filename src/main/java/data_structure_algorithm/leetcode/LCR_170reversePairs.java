...
public class LCR_170reversePairs {
    ...
    /**
     归并排序(merge过程就是还原逆序对的过程): O(NlogN), O(N)
     NOTE：合并阶段 本质上是 合并两个排序数组 的过程，而每当遇到 左子数组当前元素 > 右子数组当前元素 时，意味着 「左子数组当前元素 至 末尾元素」 与 「右子数组当前元素」 构成了若干 「逆序对」。
        对于逆序对的计数，可以采用全局变量的形式，也可以采用返回值的形式。计算逆序对时，可以计算i后面比i小的数，也可以计算j前面比j大的数，但不要同时都计算，那样就重复了。
     Ref: https://leetcode.cn/problems/shu-zu-zhong-de-ni-xu-dui-lcof/solutions/622496/jian-zhi-offer-51-shu-zu-zhong-de-ni-xu-pvn2h/
     */
