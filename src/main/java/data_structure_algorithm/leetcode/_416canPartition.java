...
public class _416canPartition {
    ...
    public static class Solution1_2 {
        ...
        public boolean canPartition(int[] nums) {
            ...
            // 回溯
            int target = sum / 2;
            visited = new boolean[nums.length][target + 1];
            return backtrace(nums, target, 0);
        }
        ...
        private boolean backtrace(int[] nums, int target, int i) {
            if (i == nums.length) return false;
            if (target == 0) return true;
            if (visited[i][target]) return false;
            visited[i][target] = true;
            // 不选第i个元素 || 选第i个元素
            return backtrace(nums, target, i + 1) || (target - nums[i] >= 0 && backtrace(nums, target - nums[i], i + 1));
        }
    ...
    public static class Solution2_3 {
